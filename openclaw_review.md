# OpenClaw 功能复盘 & X-Claw 改进指导（截至 2026-03-04）

> 目的：基于 OpenClaw 的**最新功能与近期变更**，给 X-Claw 的产品/运行时/渠道/运维/安全提供一份“可落地”的改进参考。
>
> 参考基线（本次阅读）：
> - OpenClaw：`openclaw/openclaw`，Changelog 版本 `2026.3.1 ~ 2026.3.3`（含 Breaking），本地阅读时的 commit `61f7cea`。
> - X-Claw：本仓库 `main`（对齐当时路线图；历史版本可在 git history 中追溯）。
...
对 X-Claw 来说，“吸收”的关键是：**把现有能力串成闭环**（诊断/迁移/安全/渠道 UX），而不是把 OpenClaw 的全部生态（移动节点、Canvas、超多插件）一次性搬过来。
...
## 3. 近期更新（2026.3.1 ~ 2026.3.3）对 X-Claw 最值得吸收的点
...
- 新增 `x-claw config validate`（支持 `--json`），并把校验逻辑复用到 gateway 启动前。
...
- X-Claw 已有 `migrate --from openclaw`，但 doctor 面向的是**本项目自身的长期演进**（把 breaking change 变成可控迁移）。
...
## 4. OpenClaw vs X-Claw：差距分析（截至 2026-03-04）

> 这一节把“我们已经有的能力”也写清楚，避免重复造轮子。

| 能力域 | OpenClaw（概念） | X-Claw 现状 | 差距/机会（建议方向） |
|---|---|---|---|
| 安装/引导 | onboard wizard 一条龙 | `x-claw onboard` 已有 | 强化：把“选择工具档位/安全默认/渠道授权/健康检查”也纳入引导，并沉淀 metadata 供 doctor 使用 |
...
4) **`x-claw config validate`（含 `--json`）**  
...
- PR-04：`x-claw config validate`（含 `--json` + 错误路径）
...
> 说明：本节聚焦“飞书作为企业协作主战场”时，OpenClaw 做对了哪些细节，以及 X-Claw 应该优先补哪些闭环。
...
### 9.2 X-Claw 当前飞书能力（截至 2026-03-04）
...
### 9.3 X-Claw 在飞书上“最该先补”的工程细节（优先排雷）
...
7) **多机器人群聊的误触发（@mention 更可靠）**  
   ...
   - X-Claw 可以先做轻量版：在拿到 bot info 时一并缓存 bot 显示名；当 mention.name 与 botName 不一致时不视为 @ 到自己。

### 9.4 飞书改进路线（按 PR 可落地切分）

> 以下是“对齐 OpenClaw 的关键体验”但仍符合 X-Claw 轻量定位的版本：先补闭环，再做大而全的协作工具。
