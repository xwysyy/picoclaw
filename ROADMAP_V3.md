...
后续 ports 候选（按收益排序）：
- SessionStore（会话读取/写入/快照）
- EventSink/EventStream（core 发事件，infra 订阅落盘/推送渠道/推 UI）
- Provider 协议 canonical types（messages/toolcalls/toolresults）
...
- PR-01：tool result 截断 head+tail（含单测）
...
P0（优先排雷）：
- 修复 `chat_type: "private"` 识别为 DM（否则 Lark 私聊不可用）
- mention 语义规范化（触发判断 + 上下文保真）
- outbound markdown 兼容性统一入口
- 长消息切分与长度上限
- 中文/特殊字符文件名上传兼容性 + 单测
