...
import (
	...
	"github.com/xwysyy/X-Claw/pkg/migrate/internal"
)
...
func (o *OpenclawHandler) ExecuteConfigMigration(srcConfigPath, dstConfigPath string) error {
	...
	picoCfg, warnings, err := openclawCfg.ConvertToXClaw(o.opts.SourceHome)
	if err != nil {
		return err
	}
...
func rewriteWorkspacePath(path string) string {
	path = strings.Replace(path, ".openclaw", ".x-claw", 1)
	return path
}
